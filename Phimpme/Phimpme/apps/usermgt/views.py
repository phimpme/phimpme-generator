	...
	try:
		...
		if user is not None and user.is_active:
			if request.session.get('app_name', default=None) is not None:
				orders_process(request.user, request.session['app_name'], request.session['app_package'],
				               request.session['enables'])
				request.session['app_name'] = None
				return render(request, 'success.html',
				              {'msg': 'now will continue your order...', 'url': 'cgi-bin/orders/review/'})
...
def usermgt_sendmail(to_users, key):
	send_mail('Reset Your Password!',
	          'click the url http://%s/cgi-bin/usermgt/regain/?active_key=%s' % (DOMAIN_NAME, key),
